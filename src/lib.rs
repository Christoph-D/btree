...
type Key = u32;
type NodePtr<Value, const M: usize> = NonNull<InnerOrLeafNode<Value, M>>;

/// An inner or leaf node node in a [BTree]. Layout-compatible with [InnerNode] and [LeafNode].
#[derive(Debug)]
#[repr(C)]
struct InnerOrLeafNode<Value, const M: usize> {
...
#[derive(Debug)]
#[repr(C)]
struct InnerNode<Value, const M: usize> {
    /// The number of keys is always the number of children - 1.
    /// Temporarily during modifications a node can be overfull and
    /// contain `M` keys and `M` children, see [InsertResult::Overfull].
    keys: [MaybeUninit<Key>; M],
...
#[derive(Debug)]
#[repr(C)]
struct LeafNode<Value, const M: usize> {
    /// The number of keys is always the number of children - 1.
    /// Temporarily during modifications a node can be overfull and
    /// contain `M` keys and `M` children, see [InsertResult::Overfull].
    keys: [MaybeUninit<Key>; M],
...
impl<Value, const M: usize> InnerOrLeafNode<Value, M> {
    ...
    unsafe fn format_with_indent(
        ...
        if InnerOrLeafNode::<Value, M>::is_leaf(height) {
            let keys = self
                .keys()
                .map(|k| k.to_string())
                .collect::<Vec<String>>()
...
unsafe fn split_insert<Value, const M: usize>(
    left: &mut InnerOrLeafNode<Value, M>,
...
unsafe fn inner_split_insert<Value, const M: usize>(
    left: &mut InnerNode<Value, M>,
    item_to_insert_right: NodePtr<Value, M>,
) -> (Key, NodePtr<Value, M>) {
...
unsafe fn leaf_split_insert<Value, const M: usize>(
    left: &mut LeafNode<Value, M>,
) -> (Key, NodePtr<Value, M>) {
    // Create a new sibling and move half of the children to it.
...
impl<Value, const M: usize> LeafNode<Value, M> {
    ...
    fn leak_from_box(self) -> NonNull<InnerOrLeafNode<Value, M>> {
        let n: &mut InnerOrLeafNode<Value, M> = Box::leak(Box::new(self)).into();
        NonNull::from(n)
    }
}

impl<Value, const M: usize> InnerNode<Value, M> {
...
impl<Value, const M: usize> InnerOrLeafNode<Value, M> {
    ...
    unsafe fn as_leaf_node(&self) -> &LeafNode<Value, M> {
        &*(self as *const Self as *const LeafNode<Value, M>)
    }
    /// SAFETY: Undefined behavior if this node is not a [LeafNode].
    unsafe fn as_leaf_node_mut(&mut self) -> &mut LeafNode<Value, M> {
        &mut *(self as *mut Self as *mut LeafNode<Value, M>)
    }
    /// SAFETY: Undefined behavior if this node is not an [InnerNode].
    unsafe fn as_inner_node(&self) -> &InnerNode<Value, M> {
        &*(self as *const Self as *const InnerNode<Value, M>)
    }
    /// SAFETY: Undefined behavior if this node is not an [InnerNode].
    unsafe fn as_inner_node_mut(&mut self) -> &mut InnerNode<Value, M> {
        &mut *(self as *mut Self as *mut InnerNode<Value, M>)
    }
}

impl<'a, Value, const M: usize> From<&'a mut LeafNode<Value, M>>
    for &'a mut InnerOrLeafNode<Value, M>
{
    fn from(x: &'a mut LeafNode<Value, M>) -> Self {
        // SAFETY: In memory, an [InnerOrLeafNode] is a prefix of [LeafNode].
        unsafe { &mut *(x as *mut LeafNode<Value, M> as *mut InnerOrLeafNode<Value, M>) }
    }
}

impl<'a, Value, const M: usize> From<&'a mut InnerNode<Value, M>>
    for &'a mut InnerOrLeafNode<Value, M>
{
    fn from(x: &'a mut InnerNode<Value, M>) -> Self {
        // SAFETY: In memory, an [InnerOrLeafNode] is a prefix of [InnerNode].
        unsafe { &mut *(x as *mut InnerNode<Value, M> as *mut InnerOrLeafNode<Value, M>) }
    }
}
...
impl<Value, const M: usize> InnerOrLeafNode<Value, M> {
    ...
    unsafe fn insert(&mut self, height: usize, key: Key, value: Value) -> InsertResult<Value, M> {
...
unsafe fn drop_node<Value, const M: usize>(height: usize, mut node: NodePtr<Value, M>) {
    if height == 0 {
        ...
        Box::from_raw(node.as_ptr() as *mut LeafNode<Value, M>);
    } else {
        ...
        Box::from_raw(node.as_ptr() as *mut InnerNode<Value, M>);
    }
}
...
impl<Value, const M: usize> BTree<Value, M> {
    ...
    fn root_as_ref(&self) -> &InnerOrLeafNode<Value, M> {
